...
void main() {
  ...
  for(int k = 0; k < arrownum; ++k) {
    vec2 dz = uv - vec2(d * float(k) + (1. - breite) / 2.,1.);
    float alpha = length(dz) / lambda * TWO_PI;
    z += vec2(cos(alpha),sin(alpha));
  }
  float value = brightness*length(z);
  ...
  p=vec2(length(p),0);
  if(line(uv,mouse-p/2.,mouse+p/2.) < 1./u_resolution.x) 
      color=vec3(1,0,0);
  p =mouse-p/2.;
  for (int k = 0; k < arrownum; k++) {
    vec2 z = mouse - vec2(d * float(k) + (1. - breite) / 2.,1.);
    ...
    vec2 dz=R * vec2(cos(alpha),sin(alpha));
    if(line(uv,p,p+dz) < 1./u_resolution.x) 
      color=vec3(1);
    p += dz;
  }
